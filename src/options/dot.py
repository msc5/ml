...
class Timer:
    ...
    def __rich__(self):
        msg = ''
        if self._rate != 0.0 and self._rate < 1e8:
            msg = f'[blue]{self._rate:4,.2f}[reset] steps/hour'
        return msg


class StepItem (DotItem):
...
class Steps (Dot):
    ...
    def __setattr__(self, key, value):
        ...
        stepitem = object.__getattribute__(self, key)
        stepitem._time(step=True)
